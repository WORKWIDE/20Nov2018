...
class ApiModel extends MY_Model {
    ...
    public function fse_login($fse_username, $fse_password, $fse_device_id = NULL, $device_OS = NULL, $versioncode = NULL) {
        ...
        if($query->num_rows()==0){
            ...
            $this->db->where('fse_status', 1);
            $this->db->where('fse_username', $fse_username);
            $this->db->where('fse_password', md5($fse_password));
            $query = $this->db->get();
             $res = $query->row();
    ...
    public function getFSELocation($getArray) {
    
                   $data['fse_lat']= $getArray['latitude'];
                   $data['fse_long']= $getArray['langitude'];
                   $data['LocationTime']= date('Y-m-d h:i:sa');
                   $fse_id= $data['fse_id']=$getArray['userid'];
                    $locationaddress='';
                   
                    $url = 'http://maps.googleapis.com/maps/api/geocode/json?latlng='.trim($getArray['latitude']).','.trim($getArray['langitude']).'&sensor=false';
                    ...
                    if($status=="OK")
                    {
                     ...
                     $addrw= preg_replace("/^[A-Za-z0-9 ]/", "", $locationaddress);
                    $data=array('fse_lat'=> $getArray['latitude'],'fse_long'=>$getArray['langitude'],'LocationTime'=>date('Y-m-d h:i:sa'),'fse_id'=>$getArray['userid'],'locationaddress'=>$addrw);
                   
                   // print_r($data);                    exit(); 
                   $this->db->where('fse_id', $fse_id);
                   $this->db->delete('qm_fse_location'); 
                   
                 $query=  $this->db->select('id')->from(QM_FSE_LOCATION)->where('fse_id',$fse_id)->get();
                 if ($query->num_rows() > 0) {
                     $this->db->where('fse_id', $fse_id);
                    $this->db->update(QM_FSE_LOCATION, $data);
                    $id=$fse_id; 
                 }else
                 {
                      $this->db->insert(QM_FSE_LOCATION,$data);
    ...
    public function updatePassword($getdata = NULL) {
       ...
       if ($query->num_rows() == 0) {
        ...
        $this->db->from(QM_FSE_DETAILS);
        $this->db->where('fse_status', 1);
        $this->db->where('fse_password', md5(trim($getdata['oldPassword'])));
        $this->db->where('id', trim($getdata['userId']));
        $query = $this->db->get();
         $res = $query->row();
       }else
       {
        ...
        if ($query->num_rows() == 1) {
            ...
            $datar['password'] = md5($getdata['newPassword']);
    ...
    public function resetPassword($emailid = NULL, $userId = NULL) {
        ...
        if ($query->num_rows() == 1) {
            ...
            $datas = array('fse_password' => md5($newPass));
            $this->db->where('id', $id);
    ...
    public function taskCount($userID = NULL) {
       ...
       $this->db->order_by(QM_STATUS_TYPE . '.id', 'ASC');
        $query = $this->db->get();
...
//          unset($result[0]['task_location']);
    ...
    public function inprogessTaskId($id = NULL) {
        if ($id != NULL) {
            $this->db->select('id,resume_option');
            $this->db->from(QM_TASK);
    ...
    public function fseTaskFeedbackCustSign($userID = NULL, $taskid = NULL, $fsefeedback = NULL, $custSign = NULL, $fseRating = NULL,$custComment=NULL) {



        if ($fsefeedback == NULL) {
            $data = array('customer_sign' => $custSign, 'fseRating' => $fseRating,'fse_task_comments'=>$custComment);
        } else {
            $data = array('fse_feedback' => $fsefeedback, 'customer_sign' => $custSign, 'fseRating' => $fseRating,'fse_task_comments'=>$custComment);
        }
...
*/      
       if($statusid==3)
       {
            $data = array('task_accept_date'=>$current_data,'status_id' => $statusid);
       }else{
           $data = array('end_date'=>$current_data,'status_id' => $statusid); 
       }
...
public function SendEmailupdatedtaskStatus($taskid, $statusid)
{
    $whereClause= "emialfield!=''";
    $GetEmaildata=$this->db->select('id,task_id,task_type_id,ent_id,task_report_id,email')
                           ->from(QM_TASK_REPORT_EMAILS) 
                           ->where('task_id',$taskid)
                           ->where($whereClause) 
                           ->get();
    if($GetEmaildata->num_rows() > 0)
    {
    return $GetEmaildataRes=$GetEmaildata->result();
    }
    else
    {
        return $GetEmaildataRes='0';
    }
...
*/        
        $data = array('end_date'=>date('Y-m-d H:i:s'), 'status_id' => $status, 'fse_reason`' => str_replace("#&$", " ", $reason), 'fse_task_comments' => str_replace("#&$", " ", $comment));
...
//- start --- new code added------------------------
    public function updateTaskCustomerDocumentCount($getArray = NULL) {
        $this->db->select(QM_TASK_CUSTOMER_DOCUMENT.'.task_id,'.QM_TASK_CUSTOMER_DOCUMENT.'.count,'.QM_TASK_CUSTOMER_DOCUMENT.'.id'); 
        $this->db->from(QM_TASK_CUSTOMER_DOCUMENT);
        $this->db->where(QM_TASK_CUSTOMER_DOCUMENT.'.task_id', $getArray['task_id']); 
        $query= $this->db->get(); 
        if ($query->num_rows()>0) {
    ...
    public function updateTaskCustomerDocument($getArray = NULL) {
        ...
        $data['latitude']=$getArray['latitude'];
        $data['langitude']=$getArray['langitude'];
        $data['created_date']=date("Y-m-d h:i:s");  
        $data['task_id'] = $getArray['task_id'];
        $data['offline_mode'] = $getArray['offline_mode'];
        if($getArray['upload_from_gallary']!=3)
        {
          $data['upload_from_gallary'] = $getArray['upload_from_gallary'];
        }
        $data['count'] = $count;
        if (isset($getArray['imageId'])) {
             $imageId= $getArray['imageId']; 
        }
      
        $taskid= $getArray['task_id']; 
        if($imageId!='') {
          $this->db->where('task_id',$taskid);
          $this->db->where('id',$imageId);
          $this->db->update(QM_TASK_CUSTOMER_DOCUMENT,$data);
          $this->attachment_lable_information($getArray,$imageId);
        }else{
              $this->InsertData(QM_TASK_CUSTOMER_DOCUMENT, $data);
               $insert_id =  $this->db->insert_id();
               $this->attachment_lable_information($getArray,$insert_id); 
        }
        if ($insert_id) {
            return $insert_id;
        } else {
    ...
    public function attachment_lable_information($getArray=NULL,$imgid='')
    {  
        
    if (isset($getArray['attachmentInfo'])&& $getArray['attachmentInfo']!='[]') {
        $this->db->select(QM_TASK.'.task_type_id');
        $this->db->from(QM_TASK); 
        $this->db->where(QM_TASK.'.id',$getArray['task_id']); 
         $query= $this->db->get(); 
          $result= $query->result_array();
        foreach ($getArray['attachmentInfo'] as $valp) {
            
            foreach ($valp as $key => $value) {

                    $this->db->from(QM_ATTACHMENT_ATTR_UPDATE_VALUE);
                    $this->db->where(QM_ATTACHMENT_ATTR_UPDATE_VALUE.'.update_atr_id',$key);
                    $this->db->where(QM_ATTACHMENT_ATTR_UPDATE_VALUE.'.task_dacument_id',$imgid);
                    $query= $this->db->get(); 
              ...
              if($query->num_rows()>0){
                ...
                { 
                    $attachvalue['type_id']= $result[0]['task_type_id']; 
                    $attachvalue['latitude']=$getArray['latitude'];
                    $attachvalue['langitude']=$getArray['langitude'];           
                    $attachvalue['update_atr_id']=$key;
                    $attachvalue['task_id']=$getArray['task_id'];
                    $attachvalue['value']= $strvale=  implode(',', $value); 
                    $this->db->where(QM_ATTACHMENT_ATTR_UPDATE_VALUE.'.update_atr_id',$key);
                     $this->db->where(QM_ATTACHMENT_ATTR_UPDATE_VALUE.'.task_dacument_id',$imgid);
                    $this->db->Update(QM_ATTACHMENT_ATTR_UPDATE_VALUE,$attachvalue);
                }else if($value){
                ...
                {
                    $attachvalue['type_id']=$result[0]['task_type_id'];
                    $attachvalue['task_dacument_id']=$imgid; 
                    $attachvalue['latitude']=$getArray['latitude'];
                    $attachvalue['langitude']=$getArray['langitude'];    
                    $attachvalue['update_atr_id']=$key;
                    $attachvalue['task_id']=$getArray['task_id'];
                    $attachvalue['value']= $strvale=  implode(',', $value); 
                    $this->db->insert(QM_ATTACHMENT_ATTR_UPDATE_VALUE,$attachvalue);
                }else if($value){
    ...
    public function updateTask($data) {
        ...
        if (isset($data['priority'])) {
            ...
            if (trim($data['priority']) == 'LOW') {
                ...
                $return_result['priority'] = array('priority using wrong code .. Use below status Code', 'LOW', 'MEDIUM', 'HIGH', 'CRITICAL', 'PLANING');
            }
            if ($priority != NULL) {
    ...
    public function CreateTask($data) {
        ...
        if (isset($data['priority'])) {
            ...
            if (trim($data['priority']) == 'LOW') {
                ...
                return array('priority using wrong code .. Use below status Code', 'LOW', 'MEDIUM', 'HIGH', 'CRITICAL', 'PLANING');
            }
        }
    ...
    public function CreateTasks($data) {
        ...
        if (isset($data['priority'])) {
            ...
            if (trim($data['priority']) == 'LOW') {
                ...
                $priority = 1;
            } elseif (trim($data['priority']) == 'PLANING') {
                $priority = 5;
            } else {
                return array('priority using wrong code .. Use below status Code', 'LOW', 'MEDIUM', 'HIGH', 'CRITICAL', 'PLANING');
            }
...
/*end old code---------------------------    
*/
//-start -- added/changes some line of code by dev.com
    public function pushImageToSN() {
        
        
        $task_log = array('task_log' => "",
                'task_name' => "Check",
        ...
        $this->db->select(QM_TASK_CUSTOMER_DOCUMENT . '.id,'
                ...
                . QM_TASK . '.task_name,'
                . QM_TASK . '.sys_id,'
                . QM_ENTITY . '.tp_document_upload,'
                . QM_ENTITY . '.tp_method,'
                . QM_ENTITY . '.tp_username,'
                . QM_ENTITY . '.tp_password,'
                . QM_ENTITY . '.tp_endpoint,'
                . QM_ENTITY . '.tp_apikey,'
        );
        $this->db->from(QM_TASK_CUSTOMER_DOCUMENT);
        $this->db->join(QM_TASK, QM_TASK_CUSTOMER_DOCUMENT . '.task_id = ' . QM_TASK . '.id', 'LEFT');
        $this->db->where(QM_TASK_CUSTOMER_DOCUMENT . '.upload_check != ', 1);
        $this->db->join(QM_TASK_TYPE, QM_TASK_TYPE . '.id = ' . QM_TASK . '.task_type_id');
        $this->db->join(QM_ENTITY, QM_TASK_TYPE . '.ent_id = ' . QM_ENTITY . '.id');
        $this->db->where(QM_TASK . '.sys_id !=', NULL);
        $query = $this->db->get();
        $result = $query->result_array();
...
//-end-- added/changes some line of code by dev.com */
...
//-start -- added/changes some line of code by dev.com
    public function pushCustomerSignToSN() {
        ...
        $this->db->join(QM_TASK_TYPE, QM_TASK_TYPE . '.id = ' . QM_TASK . '.task_type_id');
        $this->db->join(QM_ENTITY, QM_TASK_TYPE . '.ent_id = ' . QM_ENTITY . '.id');
        $this->db->where(QM_TASK.'.customer_sign_upload != ', 1);
        $this->db->where(QM_TASK.'.sys_id !=', NULL);
        $this->db->where(QM_TASK.'.customer_sign !=', NULL);
        $query = $this->db->get();
        $result = $query->result_array();
        return $result;
    }
//-end-- added/changes some line of code by dev.com
        ...
        if (!empty($result)) {
            foreach ($result as $value) {
                ...
                //$value = array_merge($value, $a3);
                 $attachment_status = $this->Enable_attachment_status($value['id']); 
                if ($attachment_status==1) {
                    $value['attachment_info']=$this->getattachmentlable($value['task_type_id']); 
                }else{
    ...
    public function getattachmentlable($task_typeid='')
    {
        $this->db->from(QM_ATTACHMENT_ATTR_UPDATE); 
        $this->db->where(QM_ATTACHMENT_ATTR_UPDATE.'.task_type',$task_typeid); 
        $result= $this->db->get()->result_array(); 
          foreach ($result as $key => $aa) {
                if ($aa['option_type']=='CHECKBOX'||$aa['option_type']=='RADIO'||base64_encode($aa['option_type'])=="U0VMRUNU") {
                  $result[$key]['type_values']=explode(',',str_replace('[','',str_replace('"','',str_replace(']','',$aa['type_values']))));
                }
            }
...
//        }
        ...
        $this->db->join(QM_ENTITY, QM_EXTRA_ATTR_UPDATE . '.ent_id = ' . QM_ENTITY . '.id', 'LEFT');
        ...
        foreach ($result as $r) {
            ...
            if (base64_encode($r['option_type']) == "U0VMRUNU" && strtolower($r['ent_name'])=='nashua') {
                $data[$i]['type_values'] = $this->selectOptionValueOffline($r['id'], $product_line, $r['depondid']);
            } else {
    ...
    public function selectOptionValueOffline($id = NULL, $search = NULL, $depondid = NULL) {

/* old code
        $this->db->select('option_value');
*/

// new code added by dev.com
        $this->db->select('DISTINCT(option_value)');
// end new code
        ...
        if ($query->num_rows() > 0) {
            ...
            $i = 0;
            foreach ($result as $r) {
                $data[$i] = str_replace("'", "#&&#", $r['option_value']);
...
*/
// new code added by dev.com
	$this->db->select('DISTINCT(option_value), depond_value');
// end new code
...
//        $this->db->select(QM_TASK.'.id,'.QM_TASK.'.task_type_id,'.QM_TASK.'.ent_id,'); 
//        $this->db->from(QM_TASK); 
//        $this->db->where(QM_TASK.'.id', $taskid); 
//        $query= $this->db->get(); 
//        return $query->result_array(); 
//        
//    }
    public function taskCompleted_notification_email($taskid='')
    {       
           $taskdata=array(); 
           $tasktype=array(); 
           $document=array(); 
          ...
          $taskdata = $this->TaskModel->updateTask($taskid);
          if($taskdata){
           ...
           $SendEmaildata = $this->SendEmailupdatedtaskStatus($taskid, 4); 
           // print_r($SendEmaildata);           exit();
    ...
    public function casorOfflineSave($result = NULL) {
        ...
        if (isset($result->id)) {
            ...
            if ($status_id==4) {
              $this->taskCompleted_notification_email($task_id); 
            }
            $this->db->where(QM_TASK . '.id', $task_id);
...
//            print_r($z_category_data);
    ...
    public function attachmentTaskScreen($taskid = NULL, $cat_id = NULL) {
        $product_line=NULL; 
        ...
        $this->db->from(QM_ATTACHMENT_ATTR_UPDATE);
        $this->db->join(QM_TASK, QM_TASK . '.task_type_id = ' . QM_ATTACHMENT_ATTR_UPDATE . '.task_type', 'LEFT'); 
         $this->db->join(QM_ENTITY, QM_ATTACHMENT_ATTR_UPDATE . '.ent_id = ' . QM_ENTITY . '.id', 'LEFT');   
        $this->db->where(QM_TASK . '.id =', $taskid);
        $query = $this->db->get();
        $result = $query->result_array();
        $data = array();
        ...
        foreach ($result as $r) {
            ...
            $this->db->select(QM_ATTACHMENT_ATTR_UPDATE_VALUE . '.value');
            $this->db->from(QM_ATTACHMENT_ATTR_UPDATE_VALUE);
            $this->db->where('update_atr_id', $r['id']);
            $this->db->where('task_id', $taskid);
            $this->db->order_by(QM_ATTACHMENT_ATTR_UPDATE_VALUE . '.id', 'DESC');
            $this->db->limit('1');
            $querys = $this->db->get();
            if ($querys->num_rows() == 1) {
                $post_values = $querys->row()->value;
    ...
    public function attachmentinformaiton($getarray='')
    {
        $result23= $this->db->select(QM_TASK.'.task_type_id,'.QM_TASK.'.ent_id')->from(QM_TASK)->where(QM_TASK.'.id',$getarray['task_id'])->get()->result_array(); 
        $task_type_id=  $result23[0]['task_type_id'];
        $entity_id=  $result23[0]['ent_id'];
             $this->db->select(QM_ATTACHMENT_ATTR_UPDATE.'.id,'.
            ...
            $this->db->from(QM_ATTACHMENT_ATTR_UPDATE);
            $this->db->where(QM_ATTACHMENT_ATTR_UPDATE.'.task_type',$task_type_id);
            $this->db->where(QM_ATTACHMENT_ATTR_UPDATE.'.ent_id',$entity_id);
            $query= $this->db->get(); 
            $lableresult= $query->result_array(); 
           // print_r($lableresult);            exit(); 
            if($lableresult){
                foreach($lableresult as $key=>$value){
                    $this->db->select(QM_ATTACHMENT_ATTR_UPDATE_VALUE.'.update_atr_id,'.
                        QM_ATTACHMENT_ATTR_UPDATE_VALUE.'.task_id,'.
                        QM_ATTACHMENT_ATTR_UPDATE_VALUE.'.value,'.
                        QM_ATTACHMENT_ATTR_UPDATE_VALUE.'.task_dacument_id');
                    $this->db->from(QM_ATTACHMENT_ATTR_UPDATE_VALUE);
                    $this->db->where(QM_ATTACHMENT_ATTR_UPDATE_VALUE.'.update_atr_id', $value['id']);
                    $this->db->where('task_id', $getarray['task_id']);
                     $this->db->where('task_dacument_id', $getarray['image_id']);
                    $querys = $this->db->get();
                   $valueresult= $querys->result_array(); 
                   $lableresult[$key]['id']=$value['id']; 
                   $lableresult[$key]['task_id']=($valueresult[0]['task_id']!=null)?$valueresult[0]['task_id']:''; 
                  
                   $lableresult[$key]['task_dacument_id']=($valueresult[0]['task_dacument_id']!=null)?$valueresult[0]['task_dacument_id']:''; 
                 

                   if ($value['option_type']=='CHECKBOX') {
                     $lableresult[$key]['value'] = explode(',', $valueresult[0]['value']);
                   }else{
                     $lableresult[$key]['value']=($valueresult[0]['value']!=null)?$valueresult[0]['value']:''; 
                   }
                    if ($value['option_type']=='CHECKBOX'||$value['option_type']=='RADIO'||base64_encode($value['option_type'])=="U0VMRUNU") {
                        $checkboxvalue=explode(',',str_replace('[','',str_replace('"','',str_replace(']','',$value['type_values']))));
                         $lableresult[$key]['type_values']= ($checkboxvalue)?$checkboxvalue:'';
                    }
                }
                 return  $lableresult;
            } else {                return FALSE; }
...
//public function attachmentinformaiton($getarray='')
//  {
//       $this->db->select(QM_ATTACHMENT_ATTR_UPDATE_VALUE.'.update_atr_id as id,'.
//                        QM_ATTACHMENT_ATTR_UPDATE_VALUE.'.task_id,'.
//                        QM_ATTACHMENT_ATTR_UPDATE_VALUE.'.value,'.
//                        QM_ATTACHMENT_ATTR_UPDATE_VALUE.'.task_dacument_id,'.
//                        QM_ATTACHMENT_ATTR_UPDATE.'.label,'.
...
//                        QM_ATTACHMENT_ATTR_UPDATE.'.type_values');
//       $this->db->from(QM_ATTACHMENT_ATTR_UPDATE_VALUE);      
//       $this->db->join(QM_TASK_CUSTOMER_DOCUMENT, QM_ATTACHMENT_ATTR_UPDATE_VALUE . '.task_dacument_id = ' . QM_TASK_CUSTOMER_DOCUMENT . '.id', 'LEFT');
//        $this->db->join(QM_ATTACHMENT_ATTR_UPDATE, QM_ATTACHMENT_ATTR_UPDATE_VALUE . '.update_atr_id = ' . QM_ATTACHMENT_ATTR_UPDATE . '.id', 'LEFT');
//       $this->db->where(QM_ATTACHMENT_ATTR_UPDATE_VALUE . '.task_dacument_id', $getarray['image_id']);
//       $this->db->where(QM_ATTACHMENT_ATTR_UPDATE_VALUE . '.task_id', $getarray['task_id']);
//         $query = $this->db->get();
//         if ($query->num_rows()>0) {
//             $a= $query->result_array();
//             $i=0;
//                foreach ($a as $aa) {
//                if ($aa['option_type']=='CHECKBOX') {
//                    $a[$i]['value'] = explode(',', $aa['value']);
//                   
//                }
//                if ($aa['option_type']=='CHECKBOX'||$aa['option_type']=='RADIO'||base64_encode($aa['option_type'])=="U0VMRUNU") {
//                  $a[$i]['type_values']=explode(',',str_replace('[','',str_replace('"','',str_replace(']','',$aa['type_values']))));
//                }
//
//                  $i++; 
//             }
...
//    }
    ...
    public function Enable_attachment_status($task_id='')
    {
        $status=0;

      $result23= $this->db->select(QM_TASK.'.task_type_id,'.QM_TASK.'.ent_id')->from(QM_TASK)->where(QM_TASK.'.id',$task_id)->get()->result_array(); 
        ...
        $this->db->from(QM_TASK_TYPE);
        $this->db->where(QM_TASK_TYPE.'.id', $task_type_id);
        $this->db->where(QM_TASK_TYPE.'.ent_id',$entity_id);
        $result233=   $this->db->get()->result_array(); 
        $ingreteIpi= json_decode($result233[0]['integrated_api']);         
       foreach ( $ingreteIpi as $key => $value) {
           if ($key=='attachment_status') {
              $status= $value; 
           }
       }
 ...
 public function change_fse_dutyMode($id, $duty_mode)
    {
        
          $data['duty_mode']= $duty_mode; 
          $this->db->where('id', $id); 
          $this->db->update(QM_FSE_DETAILS,$data); 
    ...
    public function last_update_screen($userid,$taskid,$resume_option)
    {

          $update_data['resume_option']= $resume_option; 
          $this->db->where(array('id'=>$taskid,'fse_id'=>$userid)); 
          $this->db->update(QM_TASK, $update_data); 
           if($this->db->affected_rows()){
              return true;             
            } else{
